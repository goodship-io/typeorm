...
exports.DuckDBQueryRunner = void 0;
 ...
 */
class DuckDBQueryRunner extends BaseQueryRunner_1.BaseQueryRunner {
    // -------------------------------------------------------------------------
    // Constructor
    // -------------------------------------------------------------------------
    ...
    stream(query, parameters, onEnd, onError) {
        throw new error_1.TypeORMError(`Stream is not supported by DuckDB driver.`);
    }
    /**
    ...
    async renameTable(oldTableOrName, newTableName) {
        const oldTableName = InstanceChecker_1.InstanceChecker.isTable(oldTableOrName)
            ? oldTableOrName.name
            : oldTableOrName;
    ...
    async renameColumn(tableOrName, oldTableColumnOrName, newTableColumnOrName) {
        ...
        const newColumn = InstanceChecker_1.InstanceChecker.isTableColumn(newTableColumnOrName)
    ...
    async createExclusionConstraint(tableOrName, exclusionConstraint) {
        ...
        throw new error_1.TypeORMError("DuckDB does not support exclusion constraints");
    }
    /**
    ...
    insertViewDefinitionSql(view) {
        return new Query_1.Query(`INSERT INTO "${this.getTypeormMetadataTableName()}" ("type", "name", "value") VALUES ($1, $2, $3)`, [MetadataTableType_1.MetadataTableType.VIEW, view.name, view.expression]);
    }
    ...
    deleteViewDefinitionSql(viewOrPath) {
        const viewName = InstanceChecker_1.InstanceChecker.isView(viewOrPath)
            ? viewOrPath.name
            : viewOrPath;
        return new Query_1.Query(`DELETE FROM "${this.getTypeormMetadataTableName()}" WHERE "type" = $1 AND "name" = $2`, [MetadataTableType_1.MetadataTableType.VIEW, viewName]);
    }
...
exports.DuckDBQueryRunner = DuckDBQueryRunner;
